...
- [x] POST /workspaces/:id/logs - 생성
- [x] GET /workspaces/:id/logs - 목록 (필터, 페이지네이션)
- [x] PATCH /workspaces/:id/logs/:logId - 수정
- [x] DELETE /workspaces/:id/logs/:logId - 삭제

### 6.3 대시보드 통계 API

- [x] GET /workspaces/:id/stats - 통계 데이터
  - [x] 일자별 지출/수입 데이터 (선 차트용)
  - [x] 수단별 지출 데이터 (파이 차트용)
  - [x] 카테고리별 지출/수입 데이터 (파이 차트용)
  - [x] 사용자별 지출/수입 데이터 (표용)

### 6.4 프론트엔드 페이지

- [x] 워크스페이스 대시보드 (/workspace/:id)
  - [x] 상단 필터 영역 (기간, 사용자, 수단, 카테고리)
  - [x] 일자별 Log 그래프 (선 차트)
  - [x] 수단별 Log 그래프 (파이 차트)
  - [x] 카테고리별 지출/수입 (파이 차트)
  - [x] Log 목록 (무한 스크롤)
...
- [x] 기본 날짜 필터를 현재 월(1일~말일)로 설정
- [x] 날짜 범위 선택 모달 구현
  - [x] 월별 선택 기능 (연도별 그룹화)
  - [x] 미래 월 disable 처리
  - [x] 연도 네비게이션 (이전 연도 이동 가능)
  - [x] 직접 범위 선택 기능 (모달 내 expand/collapse)
